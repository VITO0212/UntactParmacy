...
public class DeleteUserController implements Controller {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception {
		...
		if ((UserSessionUtils.isLoginUser("somsom", session) && 	// 로그인한 사용자가 관리자이고 	
			 !deleteId.equals("somsom"))							// 삭제 대상이 일반 사용자인 경우, 
			   || 												// 또는 
			(!UserSessionUtils.isLoginUser("somsom", session) &&  // 로그인한 사용자가 관리자가 아니고 
			...
			if (UserSessionUtils.isLoginUser("somsom", session))	// 로그인한 사용자가 관리자 	
				return "redirect:/admin/list";		// 사용자 리스트로 이동
